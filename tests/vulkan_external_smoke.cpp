...
void run_case(
    dav2::VulkanContext& context,
    dav2::GpuPreprocessor& preprocessor,
    ID3D12Device* device,
    const dav2::VulkanSemaphore& wait,
    bool rgba) {
    constexpr std::uint32_t width = 19;
    constexpr std::uint32_t height = 13;
    const dav2::ImageShape destination{28, 28};
    ...
    dav2::VulkanBuffer output = context.create_device_buffer(
    ...
    preprocessor.run(
        ...
        destination.width,
        destination.height,
        rgba ? dav2::GpuPixelOrder::rgba
             : dav2::GpuPixelOrder::bgra,
        &wait);
...
int main() {
    ...
    dav2::VulkanSemaphore wait =
        context.import_d3d12_fence(fence_handle, 1);
    CloseHandle(fence_handle);
    fence.Reset();

    dav2::GpuPreprocessor preprocessor(context);
