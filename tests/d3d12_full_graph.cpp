...
std::filesystem::path model_path() {
    if (const char* configured = std::getenv("DAV2_MODEL")) {
        return configured;
    }
...
dav2_encoder model_encoder(const std::filesystem::path& model) {
...
std::string harness_parameters(dav2_encoder encoder) {
    const char* name = encoder == DAV2_ENCODER_VITL ? "vitl" :
...
int main() try {
    ...
    dav2_create_options options{
        sizeof(options), DAV2_ABI_VERSION, model_encoder(model), 0, 0};
    ...
    // Exercise the same texture/fence path through the public InferBridge ABI.
    ibrh_api harness{};
    check(
        ibrh_get_api(
    ...
    char device_json[40]{};
    int device_length = std::snprintf(
    ...
    harness_resource.native_handle_type =
    ...
    harness_wait.native_handle_type =
        IBRH_NATIVE_HANDLE_WIN32_SHARED;
    ...
    check(
        harness.job_poll(
            harness_job, sizeof(harness_status), &harness_status),
    ...
    const std::vector<float> harness_depth = read_depth_texture(
        device.Get(), queue.Get(), harness_native_output);
