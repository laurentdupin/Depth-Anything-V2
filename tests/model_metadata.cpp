...
void write_fixture(
    ...
    FileHeader header{};
    std::memcpy(header.magic, "DAV2MOD", 7);
    header.version = 1;
    header.endian = 0x01020304;
    header.encoder = DAV2_ENCODER_VITS;
    ...
    if (include_metadata) {
        ...
        std::memcpy(metadata.magic, "DAV2META", 8);
        metadata.version = 1;
...
int main() {
    ...
    try {
        ...
        dav2::ModelFile derived_model(
            derived.u8string(), DAV2_ENCODER_VITS);
        ...
        assert(
            derived_model.derivation().encoder ==
            DAV2_ENCODER_VITS);
        ...
        dav2::ModelFile legacy_model(
            legacy.u8string(), DAV2_ENCODER_VITS);
