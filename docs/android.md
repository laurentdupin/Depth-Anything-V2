...
The native Depth Anything V2 runtime can execute as an ordinary Android
...
2. incrementally builds the arm64 shared library and `dav2_run`;
3. uploads only to `/data/local/tmp/dav2-standalone`;
4. executes one full Vulkan inference; and
5. retrieves the float32 depth map under `.BuildAndroid/evidence`.

`validation/depth_anything_v2_vits.dav2` and
