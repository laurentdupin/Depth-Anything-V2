...
public:
    ...
    void retire(dav2_gpu_job* job) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
            if (!exited_) { retired_.push_back(job); job = nullptr; }
        }
        condition_.notify_one();
        if (job) dav2_gpu_job_release(job);
    }
...
private:
    void run() noexcept {
        for (;;) {
            ...
            std::deque<dav2_gpu_job*> retired;
            {
            ...
            for (auto* old : retired) dav2_gpu_job_release(old);
            if (!job) continue;
    ...
    std::deque<dav2_gpu_job*> retired_;
    bool exited_ = false;
...
ibrh_job::~ibrh_job() {
        ...
        if (gpu_job) {
            ...
            else dav2_gpu_job_release(gpu_job);
            gpu_job = nullptr;
        }
#else
        if (gpu_job) dav2_gpu_job_release(gpu_job);
#endif
#if defined(DAV2_INFERBRIDGE_NATIVE_GPU_TEXTURES)
        gpu_admission.reset();
#endif
...
struct LinuxCaptureHooks {
    static ibr_linux_capture_capabilities capabilities(ibrh_model* model) {
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        return dav2_linux_capture_capabilities(model->context);
    }
    static void infer(ibrh_model* model, const inferbridge::linux_capture::LinuxDmaBufImage& source,
        ...
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        dav2_infer_linux_capture(model->context,source,size,output);
    }
};
}
