...
SharedD3D12Output create_shared_texture_output(
    ...
    check_hresult(
        device->CreateCommittedResource(
            ...
            nullptr,
            IID_PPV_ARGS(&result.resource)),
        "CreateCommittedResource(DAV2 texture output)");
    check_hresult(
        device->CreateFence(
            0,
            D3D12_FENCE_FLAG_SHARED,
            IID_PPV_ARGS(&result.fence)),
        "CreateFence(DAV2 texture output)");
    check_hresult(
        device->CreateSharedHandle(
            result.resource.Get(),
            nullptr,
            ...
            nullptr,
            &result.resource_handle),
        "CreateSharedHandle(DAV2 texture output)");
    check_hresult(
        device->CreateSharedHandle(
            result.fence.Get(),
            nullptr,
            ...
            nullptr,
            &result.fence_handle),
        "CreateSharedHandle(DAV2 texture output fence)");
    return result;
}
...
void validate_shared_depth_texture(
    ...
    check_hresult(device->OpenSharedHandle(
        ...
        "OpenSharedHandle(DAV2 depth output)");
    const D3D12_RESOURCE_DESC description = resource->GetDesc();
    if (description.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
...
public:
    ...
    void infer_resized(
        ...
        if (tokens <= 2000) {
            encoder_.prepare(
                ...
                static_cast<std::uint32_t>(height));
            dpt_.prepare(
                static_cast<std::uint32_t>(width / 14),
                static_cast<std::uint32_t>(height / 14));
    ...
    void infer_linux_capture(const inferbridge::linux_capture::LinuxDmaBufImage& source,
        ...
        inferbridge::linux_capture::infer_transformer(context_,source,width,height,output,
            ...
            [&] { encoder_.prepare(width,height); dpt_.prepare(width/14,height/14); });
    }
#endif
        ...
        try {
            ...
            VulkanSubmission submission = context_.batch_async(
                ...
                [&] {
                    ...
                    operators_.reduce_minmax(depth.buffer, range, count);
                    if (model_.derivation().metric_max_depth > 0.0f) {
                        ...
                        // interval into 0..1.
                        operators_.normalize_metric(
                            depth.buffer, range, count, 25.0f);
                    } else {
                        operators_.normalize_relative(
                            depth.buffer, range, count);
                    }
                    // Min/max normalization is affine and bilinear sampling is
                    // convex, so normalize at the network resolution and only
                    // then expand into the presentation resource. This is
                    // mathematically equivalent while avoiding two full-frame
                    // compute passes.
                    operators_.bilinear_align_true(
                        output,
                        depth.buffer,
