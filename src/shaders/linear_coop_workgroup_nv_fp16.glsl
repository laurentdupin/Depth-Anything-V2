...
void main(){
 ...
 tensorLayoutNV<2,gl_CooperativeMatrixClampModeConstantNV> la=
  createTensorLayoutNV(2,gl_CooperativeMatrixClampModeConstantNV);
 tensorLayoutNV<2,gl_CooperativeMatrixClampModeConstantNV> lw=
  createTensorLayoutNV(2,gl_CooperativeMatrixClampModeConstantNV);
 tensorLayoutNV<2,gl_CooperativeMatrixClampModeConstantNV> lo=
  createTensorLayoutNV(2,gl_CooperativeMatrixClampModeConstantNV);
 tensorLayoutNV<2,gl_CooperativeMatrixClampModeRepeatNV> lb=
  createTensorLayoutNV(2,gl_CooperativeMatrixClampModeRepeatNV);
 ...
 for(uint k0=0;k0<p.ic;k0+=TILE_K){
  ...
  coopMatLoadTensorNV(ma,i.d,0,sliceTensorLayoutNV(
   la,int(row0),TILE_M,int(k0),TILE_K));
  coopMatLoadTensorNV(mw,w.d,0,sliceTensorLayoutNV(
   lw,int(col0),TILE_N,int(k0),TILE_K),transpose);
  sum=coopMatMulAdd(ma,mw,sum);
 }

 coopmat<float,gl_ScopeWorkgroup,TILE_M,TILE_N,gl_MatrixUseAccumulator> bias;
 coopMatLoadTensorNV(bias,b.d,0,sliceTensorLayoutNV(
  lb,0,TILE_M,int(col0),TILE_N));
 sum+=bias;
 coopMatStoreTensorNV(sum,o.d,0,sliceTensorLayoutNV(
  lo,int(row0),TILE_M,int(col0),TILE_N));
}
