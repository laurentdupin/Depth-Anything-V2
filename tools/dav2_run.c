...
static int parse_encoder(const char* text, dav2_encoder* encoder) {
    if (strcmp(text, "vits") == 0) {
        *encoder = DAV2_ENCODER_VITS;
...
int main(int argc, char** argv) {
    ...
    dav2_status status = dav2_create(argv[1], &options, &context);
